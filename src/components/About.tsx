  ...
  return (
    <section id="about" className="section-padding bg-secondary" ref={ref}>
      ...
      >
        ...
        <motion.h2 variants={fadeUp} className="section-title mb-8">
          15+ Years of<br />Industrial Excellence
        </motion.h2>
        <motion.p variants={fadeUp} className="text-muted-foreground max-w-2xl text-sm md:text-base leading-relaxed mb-12">
          TitanDemo is a full-service demolition, debris removal, and scrap recycling company. 
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {reasons.map((r, i) => (
            <motion.div
              key={r.title}
              variants={fadeUp}
              className="bg-card p-6 border border-border card-hover"
            >
              <r.icon className="text-primary mb-4" size={28} />
              <h3 className="font-heading text-lg tracking-wider mb-2">{r.title}</h3>
